...
class serialPlot:
    def __init__(self, serialPort='/dev/ttyUSB0', serialBaud=38400, plotLength=100, numberOfChannels=3, delimiter=",", saveDir="runs", delay=5, pause=2, sampleRate = 500):
        ...
        try:
            self.serialConnection = serial.Serial(serialPort, serialBaud, timeout=4, xonxoff=False, rtscts=False, dsrdtr=False)
    ...
    def parseBuffer(self):
        ...
        while True:
            ...
            try:
                ...
                #print("Buffer empty")
                if not self.isRun and len(self.buffer)==0:
                    break
                sleep(0.001)
                continue
